...
import npuzzle.classes.{ Board, Direction, Down, Left, Move, Right, State, Up }
...
trait Bot {
  ...
  def random(state: State, incrementMoveCount: Boolean = true): State = {
    ...
    var potentialMove: Either[String, npuzzle.classes.Move] = nextMoves(randomInt)

    while (potentialMove.isLeft) {
      randomInt = (randomInt + 1) % 4
      potentialMove = nextMoves(randomInt)
    }
    val move = potentialMove.getOrElse(npuzzle.classes.Move())
    val (updatedMoves, updatedMoveCount): (Seq[Move], Int) = incrementMoveCount match {
      case false => (state.moves, state.moveCount)
      case true => (state.moves ++ Seq(move), state.moveCount + 1)
    }
    state.copy(board = move.board, seed = nextSeed, moveCount = updatedMoveCount, moves = updatedMoves)
  }
