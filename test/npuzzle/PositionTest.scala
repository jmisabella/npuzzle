...
import models.behaviors.Position
import models.classes.npuzzle.Board
import org.scalatest.flatspec.AnyFlatSpec

class PositionTest extends AnyFlatSpec {

  "Position module" should "know when values do not belong to a 2x2 board" in {
  ...
  it should "know when values do not belong to a 3x3 board" in {
    case object pos extends Position
    val size = 3
    val solution: Seq[Option[Int]] = (1 until size * size).map(Some(_)) ++ Seq(None)
    val goalState = Board(solution, size)
  ...
  it should "know when values do not belong to a 4x4 board" in {
    case object pos extends Position
    val size = 4
    val solution: Seq[Option[Int]] = (1 until size * size).map(Some(_)) ++ Seq(None)
    val goalState = Board(solution, size)
  ...
  it should "know current position on a 2x2 board" in {
    case object pos extends Position
    val size = 2
    val solution: Seq[Option[Int]] = (1 until size * size).map(Some(_)) ++ Seq(None)
    val goalState = Board(solution, size)
    assert(pos.position(goalState, Some(1)) == (0, 0), "position - in 2x2 board, position of 1 is (0, 0)")
    assert(pos.position(goalState, Some(2)) == (0, 1), "position - in 2x2 board, position of 2 is (1, 0)")
    assert(pos.position(goalState, Some(3)) == (1, 0), "position - in 2x2 board, position of 3 is (0, 1)")
    assert(pos.position(goalState, None) == (1, 1), "position - in 2x2 board, position of blank tile is (1, 1)")
  }

  it should "know current position on a 3x3 board" in {
    case object pos extends Position
    val size = 3
    val solution: Seq[Option[Int]] = (1 until size * size).map(Some(_)) ++ Seq(None)
    val goalState = Board(solution, size)
    assert(pos.position(goalState, Some(1)) == (0, 0), "position - in 3x3 board, position of 1 is (0, 0)")
    assert(pos.position(goalState, Some(2)) == (0, 1), "position - in 3x3 board, position of 1 is (1, 0)")
    assert(pos.position(goalState, Some(3)) == (0, 2), "position - in 3x3 board, position of 1 is (2, 0)")
    assert(pos.position(goalState, Some(4)) == (1, 0), "position - in 3x3 board, position of 1 is (0, 1)")
    assert(pos.position(goalState, Some(5)) == (1, 1), "position - in 3x3 board, position of 1 is (1, 1)")
    assert(pos.position(goalState, Some(6)) == (1, 2), "position - in 3x3 board, position of 1 is (2, 1)")
    assert(pos.position(goalState, Some(7)) == (2, 0), "position - in 3x3 board, position of 1 is (0, 2)")
    assert(pos.position(goalState, Some(8)) == (2, 1), "position - in 3x3 board, position of 1 is (1, 2)")
